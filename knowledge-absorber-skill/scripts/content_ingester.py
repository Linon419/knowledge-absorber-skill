...
class Config:
    ...
    # OCR noise patterns to filter out (university branding, copyright, page numbers)
    OCR_NOISE_PATTERNS = [
        re.compile(r'(?i)the\s+university\s+of\s+sydney'),
        re.compile(r'(?i)copyright\s*©?\s*\d{4}'),
        re.compile(r'(?i)all\s+rights\s+reserved'),
        re.compile(r'(?i)page\s+\d+\s*(of\s+\d+)?'),
        re.compile(r'(?i)slide\s+\d+'),
        re.compile(r'(?i)comp\d{4}\s*[-–]\s*\d{4}'),  # Course codes like COMP5048-2026
        re.compile(r'(?i)school\s+of\s+(computer\s+science|information\s+technologies|it)'),
        re.compile(r'(?i)faculty\s+of\s+engineering'),
...
class ContentParser:
    ...
    def perform_ocr(self, img_path):
        ...
        try:
            ...
            if result:
                ...
                text = self._filter_ocr_noise(text)
                return text if text.strip() else "[OCR: No meaningful text found]"
            return "[OCR: No text found]"
        except Exception as e:
            return f"[OCR Error: {e}]"

    def _filter_ocr_noise(self, text):
        """Filter out repetitive university branding, copyright, and page number noise from OCR text."""
        lines = text.split('\n')
        filtered = []
        for line in lines:
            stripped = line.strip()
            if not stripped:
                continue
            is_noise = any(p.search(stripped) for p in Config.OCR_NOISE_PATTERNS)
            if not is_noise:
    ...
    def _process_pdf_page(self, page_idx, page):
        """Processes a single PDF page: text + images (OCR) with slide title detection"""
        content = ""
        # Text
        page_text = page.extract_text()
        if page_text:
            # Filter OCR noise from extracted text
            page_text = self._filter_ocr_noise(page_text)

            # Slide title detection heuristic:
            # If the first non-empty line is short (<=80 chars) and the page has more content,
            # treat it as a slide title/header
            lines = [l.strip() for l in page_text.split('\n') if l.strip()]
            slide_header = ""
            if lines:
                first_line = lines[0]
                # Short first line likely a slide title (not a sentence fragment)
                if len(first_line) <= 80 and len(lines) > 1 and not first_line.endswith((',', ';', ':', '-')):
                    slide_header = f"\n### [SLIDE {page_idx+1}] {first_line}\n"

            if slide_header:
                content += slide_header
                # Rest of page text (skip the title line we already used)
                remaining = '\n'.join(lines[1:])
                if remaining.strip():
                    content += f"{remaining}\n"
            else:
    ...
    def _extract_pptx_content(self, file_path):
        """Extract content from PPTX files: titles, body text, speaker notes, and image OCR."""
        log(f"Extracting content from PPTX: {file_path}")
        try:
        ...
        try:
            ...
            log(f"PPTX has {num_slides} slides. Processing...")
            ...
            for slide_idx, slide in enumerate(prs.slides):
                ...
                # Extract body text from all text frames (excluding title)
                body_texts = []
                ...
                if slide.has_notes_slide and slide.notes_slide.notes_text_frame:
                    ...
                    if notes_text:
                        notes_text = self._filter_ocr_noise(notes_text)
                        if notes_text.strip():
                            slide_content += f"\n[SPEAKER NOTES]:\n{notes_text}\n"
                ...
                for shape in slide.shapes:
                    if shape.shape_type == 13:  # MSO_SHAPE_TYPE.PICTURE
                        ...
                        try:
                            image = shape.image
                            ext = os.path.splitext(image.content_type.split('/')[-1])[0] or 'png'
                            with tempfile.NamedTemporaryFile(suffix=f'.{ext}', delete=False) as tmp_img:
                                tmp_img.write(image.blob)
                                tmp_img_path = tmp_img.name

                            ocr_text = self.perform_ocr(tmp_img_path)
                            if ocr_text and not ocr_text.startswith("[OCR"):
                                slide_content += f"\n[SLIDE {slide_idx+1} IMAGE {img_idx} CONTENT (OCR)]:\n{ocr_text}\n"
                            ...
                            log(f"Error extracting image from slide {slide_idx+1}: {e}")
